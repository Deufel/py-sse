...
def _(mo):
    mo.md(r"""
    stream.py — the SSE streaming module. This is where py-sse earns its name. It needs to:

    1. Start an SSE response (special headers, keep connection open)
    2. Send events (using html-tags patch_elements/patch_signals)
    3. Send keep-alive comments so the connection doesn't timeout
    4. Detect client disconnect and clean up
    """)
    return

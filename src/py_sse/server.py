...
def _envelope(head_fragments, body_inner, ui_theme="dark"):
...
def _envelope_safe(head_fragments, body_inner_html, ui_theme="dark"):
...
def live(handler=None, *, topic=None, hard_cap=None):
    ...
    def wrapper(req):
        ...
        def render_inner():
            ...
            return _render_content(handler(req))
        ...
        if is_sse_request:
            ...
            return _stream_live(t, live_counter, changes, render_inner)
        ...
        full = _envelope_safe(head_fragments, wrapper_html, ui_theme)
        return html(full)

    return wrapper

def _stream_live(topic, live_counter, changes, render_inner):

    ...
    await res.html(str(Html(
        ...
        Body(
            ...
            Div({"class": "chat-box", 
                ...
                # "data-on:visibilitychange__window": "if(!document.hidden) @get('/stream')"
                },
                render_messages()),
            Div({"class": "controls"},
...
async def stream(req, send, closed):
    ...
    await s.open(compress=compress)
    await s.send_event(patch_elements(render_messages()))
...
#uvicorn wave_brotli:app --reload